...
public class ConferenceApi {
		...
		WrappedBoolean result = ofy().transact(new Work<WrappedBoolean>() {
			public WrappedBoolean run() {
				try {
					...
					Conference conference = getConference(conferenceKey.getString());

					// 404 when there is no Conference with the given conferenceId.
					if (conference == null) {
						return new WrappedBoolean(false, "404 No Conference found with key: " + websafeConferenceKey);
					}
	...
	public Collection<Conference> getConferencesToAttend(final User user)
		...
		// If not signed in, throw a 401 error.
		if (user == null) {
			throw new UnauthorizedException("401 Authorization required");
		}
		// TODO
		// Get the Profile entity for the user
		Profile profile = getProfileFromUser(user); // Change this - DONE;
		if (profile == null) {
		...
		List<String> keyStringsToAttend = profile.getConferenceKeysToAttend(); // change this - DONE

		// TODO
		// Iterate over keyStringsToAttend,
		// and return a Collection of the
		// Conference entities that the user has registered to attend
		Collection<Conference> listOfConferenceEntities = new ArrayList<Conference>();
		for (String key : keyStringsToAttend) {
			listOfConferenceEntities.add(getConference(key));
		}
	 ...
	 *             when there is no Conference with the given conferenceId.
	 */
	@ApiMethod(name = "unregisterFromConference", path = "conference/{websafeConferenceKey}/unregistration", httpMethod = HttpMethod.DELETE)
	public WrappedBoolean unregisterFromConference(final User user,
		...
		try {
			...
			return new WrappedBoolean(false, "An error happened: " + e.getMessage());
		}
		// We are unregistered!
